...
target_file = "data/output/debug2/orig_target_final.png"
num_images = 3
canvas_files = [f"data/output/debug2/canvas_{i}.png" for i in range(num_images)]
img_size = 1000
...
def main():
    ...
    # Create a diff
    #
    clear_files(["data/output/debug3"])
    banned_idx = 0
    max_brightness = 1.0
    target_img = load_image(target_file)
    blurred_img = low_pass_filter(target_img, sigma=5)
    ...
    save_image(blurred_img, "blurred_img_10.png", image_type=SaveType.DEBUG3)
    save_image(diff, "filter_diff.png", image_type=SaveType.DEBUG3)

    labels, blobs = detect_blobs(diff)
    num_blobs = len(blobs)
    blob_img = visualise_blobs(labels, blobs)
    save_image(blob_img, "blob_img_10.png", image_type=SaveType.DEBUG3)
    ...
    for i, canvas in enumerate(canvases):
        save_image(canvas, f"canvas_{i}.png", image_type=SaveType.DEBUG3)
    combined = combine_images(canvases)
    save_image(combined, "combined_final.png", image_type=SaveType.DEBUG3)
    save_image(inc_target_img, "target_img_final.png", image_type=SaveType.DEBUG3)
    save_image(blurred_img, "blurred_img_final.png", image_type=SaveType.DEBUG3)
    save_image(target_img, "orig_target_final.png", image_type=SaveType.DEBUG3)
